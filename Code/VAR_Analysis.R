...
pdf(file = paste("/mnt/d/2022_Jan-Apr/Semester 8/Mth UGP/Code/Report_Images/VAR_GEN_DET_MIX.pdf",sep=""), height = 8, width = 10, pointsize = 10)
...
for(b in b_dist){
    ...
    min_x <- min(min_x,(c(density(det_sigma_bm[ which(b == b_dist), ])$x, density(det_sigma_est[which(b == b_dist), ])$x)))
...
for(b in b_dist){
    ...
    # plot(density(det_sigma_est[ which(b == b_dist), ]), main = paste0(" Determinants T : ", T, " Batch Size : ", b) ,lwd = 1, col ="red")
    plot(density(det_sigma_est[ which(b == b_dist), ]), xlim = c(min_x,max_x*1.20) , ylim = c(0,max_y*1.10) , main = paste0(" Determinants T : ", T, " Batch Size : ", b) ,lwd = 1, col ="blue")
    lines(density(det_sigma_bm[ which(b == b_dist), ]), lwd = 1, col = "red")
    abline(v = det_act_sigma)
    # legend("topright", legend=c("BM ", "EST "), col=c("blue","red"), lty=1:1:1, cex=0.8)
...
for(b in b_dist){
    # max_y <- max(max_y,c(density(error_bm[ which(b == b_dist), ])$y, density(error_est[which(b == b_dist), ])$y))
    # max_x <- max(max_x,c(density(error_bm[ which(b == b_dist), ])$x, density(error_est[which(b == b_dist), ])$x))
    plot(density(error_est[ which(b == b_dist), ]), xlim = c(0,max_x*1.50) , ylim = c(0,max_y*1.10) , main = paste0(" Error T : ", T, " Batch Size : ", b) ,lwd = 1, col ="blue")
    lines(density(error_bm[ which(b == b_dist), ]), lwd = 1, col = "red")
}
