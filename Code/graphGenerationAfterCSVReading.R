data <- read.csv("/mnt/d/2022_Jan-Apr/Semester 8/Mth UGP/Code/StatisticCSVFIle/phi_0_6.csv")
print(data)

vals <- matrix(0, nrow = 8, ncol = 9)
# vals[2,] <- as.numeric(data[2,])
phi_val = c(6,9,95,99)


for(x in 0:(length(phi_val)-1)){
    data <- read.csv(paste0("/mnt/d/2022_Jan-Apr/Semester 8/Mth UGP/Code/StatisticCSVFIle/phi_0_",phi_val[x+1],".csv"))
    for(y in 1:2){
        vals[(2*x) + y,] <- as.numeric(data[y,])
    }
}

pdf(file = paste("/mnt/d/2022_Jan-Apr/Semester 8/Mth UGP/Code/Report_Images/Stats_1.pdf",sep=""), height = 10, width = 8, pointsize = 10)

name <- c("Mean","Variance","MSE");
par(oma = c(4,1,1,1), mfrow = c(4, 3), mar = c(2, 2, 1, 1))
for(i in seq.int(0,7,2)){
    curr_data <- t(vals[(i+1):(i+2),])
    for(j in 0:2){
        plot_data <- curr_data[((j*3)+1):((j*3)+3),]
        barplot(plot_data, main = paste0(name[j+1], " Phi : 0.",phi_val[(i/2)+1]), ylim = range(pretty(c(0,plot_data))), col = c("red","blue","green"), names.arg = c(b_dist[1],b_dist[2]),beside = TRUE)
    }
}
